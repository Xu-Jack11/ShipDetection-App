...
class ResNetBackbone(nn.Module):
    """ResNet-style backbone returning multi-scale feature maps."""
    ...
    def __init__(
        ...
        self.out_channels = {
            "s2": base_channels * block.expansion,
            "s3": base_channels * 2 * block.expansion,
            "s4": base_channels * 4 * block.expansion,
            "s5": base_channels * 8 * block.expansion,
        }
    ...
    def forward(self, x: Tensor) -> Dict[str, Tensor]:
        ...
        return {"s2": c2, "s3": c3, "s4": c4, "s5": c5}


def build_resnet_backbone(depth: int = 50, **kwargs: Tuple) -> ResNetBackbone:
    """Factory method to align with popular ResNet depth choices."""
    config = {
    ...
    return ResNetBackbone(block=block, layers=layers, **kwargs)
